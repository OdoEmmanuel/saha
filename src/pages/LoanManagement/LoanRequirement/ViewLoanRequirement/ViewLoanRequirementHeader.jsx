...
const ViewLoanRequirementHeader = ({id}) => {
  ...
  return (
    ...
    <div className='flex items-center mb-8'>
        ...
        <Link to={`/ui/tables/loan-requirement/details/${id}/`} onClick={() => setActiveLink('customers')} className={`mr-4   pb-2 ${activeLink === 'customers' ? 'text-gray-600 font-[500]  border-b-2 border-gray-700':'text-gray-400'}`}> Loan Details </Link>
        <Link to={`/ui/tables/loan-requirement/details/${id}/filled`} onClick={() => setActiveLink('kin')} className={`mr-4  pb-2 ${activeLink === 'kin' ?  'text-gray-600 font-[500]  border-b-2 border-gray-700':'text-gray-500'}` }>Loan Requirement</Link>
