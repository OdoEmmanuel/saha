    ...
    useEffect(() => {
        if (searchQuery.trim() === '') {
            ...
            const filtered = transactions.filter((transaction) => {
                // Check each field safely with proper fixed deposit fields
                const searchableFields = [
                    ...
                    transaction.transactionDescription?.toString()?.toLowerCase() ?? '',
                    transaction.fromAccountNumber?.toString()?.toLowerCase() ?? '',
                    transaction.toAccountNumber?.toString()?.toLowerCase() ?? '',
                    transaction.retrievalRefferenceNumber?.toString()?.toLowerCase() ?? '',
                    transaction.narration?.toString()?.toLowerCase() ?? '',
